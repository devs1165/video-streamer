...
class YouTubeAuthService {
  ...
  public async getStreamKey(
    ...
    try {
      ...
      const broadcastId = await this.createBroadcast(
        newAccessToken,
        title,
        scheduleDate,
      );
      ...
      return {
        token: streamDetails.items[0].cdn.ingestionInfo.streamName,
        broadcastId,
      };
    } catch (getStreamKeyError: any) {
      console.log(
        'Error in getOrCreateStreamKey, refreshing token...',
        getStreamKeyError.response?.data?.error?.errors,
      );
  ...
  public async createBroadcast(
    ...
    try {
      ...
      console.log(
        ...
        createBroadcastError.response?.data?.error?.errors,
      );
  ...
  public async createStream(accessToken: string, title: string) {
    try {
      ...
      console.log(
        ...
        CreateStreamError.response?.data?.error?.errors,
      );
      throw new Error('Error creating stream');
    }
  }

  public async bindBroadcastToStream(
    accessToken: string,
    broadcastId: string,
    streamId: string,
  ) {
    try {
      ...
      console.log(
        `Successfully bound broadcastId ${broadcastId} to streamId ${streamId}`,
      );
    } catch (bindBroadcastToStreamError: any) {
      console.log(
        'Error binding broadcast to stream:',
        bindBroadcastToStreamError.response?.data?.error?.errors,
      );
    ...
    try {
      await HttpClient.post({
        ...
        params: {
          ...
          id: broadcastId,
          part: 'snippet,status',
          key: authConfig.youtubeAPIKey,
        },
      });
      console.log(
        `Stream with broadcastId ${broadcastId} is now live on YouTube.`,
      );
    } catch (activateStreamError: any) {
      console.error(
        'Error activating YouTube stream:',
        activateStreamError.response?.data?.error?.errors,
      );
