...
class StreamService {
  ...
  public async createStream(
    ...
    const playBackPolicy: PlaybackPolicy = visibility
      ? defaultPublicPlaybackPolicy
    ...
    try {
      ...
      stream.destinations =
        multistreamTargets?.map((destination) => destination.destinations) ||
        [];
  ...
  public async activateStream(
    ...
    if (stream?.destinations) {
      ...
      stream.destinations.forEach(async (platform) => {
        const streamingPlatform = streamTokens.filter(
          (token) => token.type == platform.platform,
        )[0];
   ...
   */
  private async activatePlatform(
    destination: StreamPlatform,
    streamingPlatform: MultiStreamToken,
  ): Promise<void> {
    const { platform, broadcastId } = destination;
    try {
      switch (platform) {
        case Platform.Youtube:
          await YoutubeService.activateStream(
            broadcastId,
            await YoutubeService.refreshAccessToken(
              streamingPlatform.token.refreshToken,
            ),
          );
          break;
        ...
        default:
          console.warn(
            `Activation for platform ${platform} is not implemented.`,
          );
  ...
  private async generateMultistreamTargets(
    ...
    for (const platform of platforms) {
      try {
        ...
        const platformData = await this.getPlatformStreamKey(
          platform,
          refreshToken,
          title,
          scheduleDate,
        ...
        multistreamTargets.push({
          target,
          destinations: { broadcastId: platformData.broadcastId, platform },
        });
      } catch (generateMultistreamTargetsError) {
        console.error(
          `Error generating multistream target for ${platform}:`,
          generateMultistreamTargetsError,
        );
      }
    }
  ...
  private async getPlatformStreamKey(
    ...
    switch (platform) {
      ...
      case Platform.Twitch:
        return await TwitchService.getStreamKey(
          refreshToken,
          title,
          scheduleDate,
        );
